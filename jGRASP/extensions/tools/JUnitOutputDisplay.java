...
public class JUnitOutputDisplay implements Runnable {
   ...
   public void run() {
      ...
      try {
         ...
         showError(tcd, "Unexpected encoding error: "
               + ToolUtil.getDetailMessage(e));
         close(serviceSocket);
         close(serverSocket);
         synchronized (this) {
         ...
         GIconData iconData = 
            new GIconData() {
               ...
               public Paint getBackgroundPaint(final int x, final int y,
                  ...
                  Color[] colors = new Color[] { new Color(255, 170, 170),
                        Color.WHITE, new Color(170, 255, 170) };
                  Paint result = PluginUtil.createLinearGradientPaint(
