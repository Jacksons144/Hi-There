...
public class JUnitDebugIcon extends AutosizedIcon {
   ...
   public void paintImage(final Toolkit tk, final Graphics2D g, final int w,
      ...
      int yoffs = (h - sz) / 2;
      if (sz != lastSz) {
         setPaints(sz, new GradientPaint(xoffs + sz / 4, yoffs + sz / 4,
               Color.RED,
               xoffs + sz / 2, yoffs + sz / 2, Color.WHITE),
               new GradientPaint(xoffs + sz - sz / 4, yoffs + sz - sz / 4,
      ...
      path.closePath();
      g.clip(path);
      Drawing.drawBug(g, 0, 0, w, h, Math.min(w, h), ulPaint);
      g.setClip(s);
      ...
      path.closePath();
      g.clip(path);
      Drawing.drawBug(g, 0, 0, w, h, Math.min(w, h), lrPaint);
      g.setClip(s);
   }
}
